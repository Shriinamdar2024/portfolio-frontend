...
import { 
  ...
  Terminal, Layout, Cpu, Sun, Moon, Sparkles
} from 'lucide-react';
        ...
        <>
                  ...
                  {data.experience.map((exp, i) => (
                    ...
                    >
                      ...
                      <div className="absolute left-0 top-0 z-10">
                        ...
                        >
                          {exp.companyLogo ? (
                            <img src={exp.companyLogo} alt={exp.company} className="w-full h-full object-cover p-1" />
                          ) : (
                            <div className="flex flex-col items-center justify-center">
                               <span className="text-[10px] font-black text-indigo-500">{exp.company?.substring(0, 2).toUpperCase()}</span>
                      ...
                      <div className={`relative p-6 rounded-3xl transition-all duration-500 ${isDarkMode ? 'hover:bg-white/[0.02]' : 'hover:bg-indigo-50/30'}`}>
                        <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4 mb-3">
                          ...
                          <p className="text-sm text-indigo-400 font-bold">{exp.company}</p>
                        </div>
                        <h4 className={`text-2xl font-black ${isDarkMode ? 'text-white' : 'text-slate-900'} mb-4 group-hover:text-indigo-400 transition-colors`}>{exp.role}</h4>
          ...
          <footer className={`${isDarkMode ? 'bg-[#050507]' : 'bg-slate-50'} pt-32 pb-16 relative overflow-hidden transition-colors`}>
            <div className="max-w-7xl mx-auto px-8">
                <motion.div initial={{ opacity: 0, y: 50 }} whileInView={{ opacity: 1, y: 0 }} className="flex flex-col items-center text-center space-y-12 mb-32">
                  ...
                  <motion.a whileHover={{ scale: 1.05 }} href={`mailto:${myEmail}`} className={`text-2xl md:text-5xl ${isDarkMode ? 'text-slate-400' : 'text-slate-500'} hover:text-indigo-500 transition-colors font-thin border-b-2 border-indigo-500/20 pb-4`}>
                    {myEmail}
                  </motion.a>
                </motion.div>
                ...
                <div className="flex flex-col md:flex-row items-center justify-between pt-16 border-t border-indigo-500/10 gap-10">
                  <div className="flex gap-12">
                    {[
                      ...
                      <motion.a key={idx} whileHover={{ y: -5, color: "#6366f1" }} href={social.link} target="_blank" rel="noreferrer" className={`${isDarkMode ? 'text-slate-600' : 'text-slate-400'} transition-colors flex items-center gap-3`}>
                        <social.icon size={18} />
